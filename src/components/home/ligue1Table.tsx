...
export default function Ligue1Table() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        if (data.franceLigue1) {
          setStandings(data.franceLigue1);
        }
      } catch (err) {
        console.error('Failed to load Ligue 1 standings');
      } finally {
        setLoading(false);
      }
    };
      ...
      <div className="bg-[#091c3e] text-white p-2 border-b border-blue-900">
        <h3 className="text-sm font-bold flex items-center gap-2">
            <Image
                ...
                width={100}
                height={100}
                className="w-10 h-10 object-contain transition-transform duration-300 group-hover:scale-120 invert" 
                priority
            />
            Ligue 1
        </h3>
